...
export function InspectorPanel({
  ...
  const [capabilities, setCapabilities] = useState<CapabilitySet | null>(model?.capabilities ?? null);
  const [configuration, setConfiguration] = useState<ModelConfiguration | null>(model?.configuration ?? null);
  ...
  const changed = useMemo(
    () => Boolean(
      ...
      && (
        ...
        || JSON.stringify(model.configuration) !== JSON.stringify(configuration)
      )
    ),
    ...
    <aside className="inspector-panel" aria-labelledby="inspector-heading">
      ...
      {model && capabilities && configuration ? (
        <div className="inspector-content">
          <section className="model-summary">
            ...
            <div>
              ...
              <span>{t("vendor")}: {model.vendor}</span>
            </div>
          </section>
          ...
          <section className="inspector-section">
            <div className="section-title-row">
              ...
              <Button variant="ghost" size="sm" type="button" onClick={onProbe} disabled={busy}>
                <Sparkles aria-hidden="true" size={16} />
                {t("probe")}
              </Button>
            </div>
            <div className="capability-list">
              <CapabilityToggle icon={<Wrench />} label={t("toolCall")} checked={capabilities.supportsToolCall} source={sourceFor(model, "toolCall", t)} onChange={(checked) => setCapabilities({ ...capabilities, supportsToolCall: checked })} />
              <CapabilityToggle icon={<Image />} label={t("images")} checked={capabilities.supportsImages} source={sourceFor(model, "images", t)} onChange={(checked) => setCapabilities({ ...capabilities, supportsImages: checked })} />
              <CapabilityToggle
                icon={<BrainCircuit />}
                label={t("reasoning")}
                ...
                onChange={(checked) => {
                  setCapabilities({ ...capabilities, supportsReasoning: checked });
                  if (!checked) {
                    setConfiguration({
                      ...configuration,
                      onlyReasoning: false,
            ...
            <details className="model-config-details">
              <summary>
                <span><SlidersHorizontal aria-hidden="true" size={16} />{t("advancedModelConfig")}</span>
                <ChevronDown className="model-config-details__chevron" aria-hidden="true" size={16} />
              </summary>
              <div className="model-config-details__content">
                ...
                <ConfigGroup title={t("invocationConfig")}>
                  <ConfigField label={t("endpointOverride")} hint={t("endpointOverrideHint")}>
                    <Input
                      type="url"
                  ...
                  <div className="model-config-grid model-config-grid--numbers">
                    <NumberField
                      ...
                      integer
                      onChange={(value) => setConfiguration({ ...configuration, maxInputTokens: value })}
                    />
                    <NumberField
                      ...
                      integer
                      onChange={(value) => setConfiguration({ ...configuration, maxOutputTokens: value })}
                    />
                    <NumberField
                      label={t("temperature")}
                      value={configuration.temperature}
                      onChange={(value) => setConfiguration({ ...configuration, temperature: value })}
                    />
                  </div>
                  <ConfigToggle
                    ...
                    onChange={(checked) => setConfiguration({ ...configuration, useCustomProtocol: checked })}
                  />
                </ConfigGroup>
          ...
          <section className="inspector-section publish-section">
            ...
            <div className="target-list">
              {targets.map((target) => {
                const invalid = !target.installed || target.schema === "invalid" || !target.writable;
                return (
                  <label
                  ...
                  >
                    ...
                    />
                    <span className="target-option__checkbox" aria-hidden="true"><Check size={13} /></span>
                    <TargetIcon target={target.kind} />
                    <span className="target-option__copy">
                      <strong>{target.displayName}</strong>
                      <small className={target.drifted || invalid ? "status-warning" : undefined}>
            ...
            <div className={`publish-readiness${selectedCount > 0 && selectedTargets.length > 0 ? " is-ready" : ""}`}>
              <GitCompareArrows aria-hidden="true" size={18} />
              <div>
                <strong>{t("publishScope", { models: selectedCount, targets: selectedTargets.length })}</strong>
        ...
        <div className="empty-state inspector-empty">
          <div className="empty-state__glyph" aria-hidden="true"><BrainCircuit size={24} /></div>
...
const reasoningEfforts: ReasoningEffort[] = ["minimal", "low", "medium", "high", "xhigh", "max"];
const reasoningSummaries: ReasoningSummary[] = ["auto", "concise", "detailed", "always", "never"];
...
function ReasoningFields({
  ...
  const toggleEffort = (effort: ReasoningEffort, checked: boolean) => {
    ...
    const supportedEfforts = checked
      ? reasoningEfforts.filter((item) => item === effort || current.includes(item))
      : current.filter((item) => item !== effort);
    updateReasoning({
      ...configuration.reasoning,
      supportedEfforts,
      effort: configuration.reasoning.effort === effort && !checked ? null : configuration.reasoning.effort,
      defaultEffort: configuration.reasoning.defaultEffort === effort && !checked
        ? null
    ...
    <ConfigGroup title={t("reasoningConfig")}>
      ...
      <ConfigToggle
        ...
        onChange={(checked) => onChange({
          ...
          reasoning: { ...configuration.reasoning, canDisableThinking: checked },
        })}
      />
          ...
          {reasoningEfforts.map((effort) => {
            ...
            return (
              <label htmlFor={id} key={effort}>
                <Checkbox
                  id={id}
                  checked={configuration.reasoning.supportedEfforts.includes(effort)}
      ...
      <div className="model-config-grid">
        <EffortSelect
          ...
          t={t}
          onChange={(value) => updateReasoning({ ...configuration.reasoning, defaultEffort: value })}
        />
        <EffortSelect
          ...
          emptyLabel={t("notSet")}
          t={t}
          onChange={(value) => updateReasoning({ ...configuration.reasoning, effort: value })}
        />
      </div>
      <ConfigField label={t("reasoningSummary")} hint={t("reasoningSummaryHint")}>
        <select
          className="model-config-select"
          value={configuration.reasoning.summary ?? ""}
          onChange={(event) => updateReasoning({
            ...configuration.reasoning,
            summary: event.target.value ? event.target.value as ReasoningSummary : null,
          })}
        >
          <option value="">{t("notSet")}</option>
          {reasoningSummaries.map((summary) => (
            <option key={summary} value={summary}>{t(`summary_${summary}`)}</option>
          ))}
        </select>
      </ConfigField>
    </ConfigGroup>
  ...
  return (
    <ConfigField label={label}>
      <select
        ...
        onChange={(event) => onChange(event.target.value ? event.target.value as ReasoningEffort : null)}
      >
        <option value="">{emptyLabel}</option>
        {supported.map((effort) => (
          <option key={effort} value={effort}>{t(`effort_${effort}`)}</option>
        ))}
      </select>
    </ConfigField>
  );
...
function TargetStatusIcon({ target }: { target: TargetStatus }) {
  if (target.schema === "invalid" || !target.writable) return <CircleAlert className="status-warning" aria-hidden="true" size={17} />;
  if (target.drifted) return <TriangleAlert className="status-warning" aria-hidden="true" size={17} />;
  if (!target.installed) return <CircleDashed aria-hidden="true" size={17} />;
  return <CircleCheckBig aria-hidden="true" size={17} />;
}

function CapabilityToggle({ icon, label, checked, source, onChange }: { icon: React.ReactElement; label: string; checked: boolean; source: string; onChange: (checked: boolean) => void }) {
  return (
    <label className="capability-toggle">
      <input
        ...
        aria-label={label}
      />
      <span className="capability-toggle__icon" aria-hidden="true">{icon}</span>
      <span className="capability-toggle__copy">
        <strong>{label}</strong>
        <small>{source}</small>
      </span>
      <span className="capability-toggle__visual-switch" aria-hidden="true"><span /></span>
    </label>
  );
}

const sourceOrder = ["manual", "probe", "imported", "metadata", "catalog", "default"] as const;

function sourceFor(model: ManagedModel, capability: string, t: ReturnType<typeof createTranslator>) {
  const evidence = sourceOrder
    .map((source) => model.evidence.find((item) => item.capability === capability && item.source === source))
    .find(Boolean);
  const labels = {
  ...
  return t("capabilitySource", { source: evidence ? labels[evidence.source] : t("unknown") });
}

function targetStatusLabel(target: TargetStatus, t: ReturnType<typeof createTranslator>) {
  if (target.schema === "invalid" || !target.writable) return t("targetInvalid");
