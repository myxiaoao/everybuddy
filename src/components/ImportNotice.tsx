...
export function ImportNotice({
  ...
  return (
    <aside className={`import-notice${hasIssues ? "" : " is-success"}`} aria-labelledby="import-notice-title">
      {hasIssues ? <AlertTriangle aria-hidden="true" size={17} /> : <CheckCircle2 aria-hidden="true" size={17} />}
      <div className="import-notice__content">
        <strong id="import-notice-title">{hasIssues ? t("importNoticeTitle") : t("importNoticeSuccessTitle")}</strong>
        {report.importedGatewayCount > 0 || report.importedModelCount > 0 ? (
          <span>{t("importSucceeded", { gateways: report.importedGatewayCount, models: report.importedModelCount })}</span>
        ) : null}
        {hasIssues ? <span>{t("importNoticeSummary", { count: report.issues.length })}</span> : null}
        {expanded ? (
          <ul>
            {report.issues.map((item, index) => (
              <li key={`${item.target}-${item.modelId ?? "target"}-${item.code}-${index}`}>
                <strong>{item.modelId ? `${displayTarget(item.target)} · ${item.modelId}` : displayTarget(item.target)}</strong>
      ...
      {hasIssues ? (
        <Button variant="ghost" size="sm" type="button" onClick={onToggle} aria-expanded={expanded}>
      ...
      <Button variant="ghost" size="icon-sm" type="button" onClick={onClose} aria-label={t("dismissImportNotice")}>
...
function importIssueLabel(code: string, t: ReturnType<typeof createTranslator>) {
  const labels = {
