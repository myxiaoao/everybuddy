...
describe("EveryBuddy workspace", () => {
  it("renders the demo gateway, models, and both configuration targets", async () => {
    ...
    await waitFor(() => expect(screen.getAllByText("GPT-5.6").length).toBeGreaterThan(0));
    ...
    expect(screen.getByRole("checkbox", { name: "选择模型 GPT-5.6" })).toBeInTheDocument();
    expect(screen.getByRole("banner", { name: "配置工作流" })).toBeInTheDocument();
    ...
    within(modelTable).getAllByRole("row").slice(1).forEach((row) => {
      expect(within(row).getAllByRole("cell")).toHaveLength(3);
    });
    expect(modelTable.querySelectorAll(".capability-icons [tabindex]")).toHaveLength(0);
    ...
    fireEvent.change(within(dialog).getByRole("textbox", { name: /Model ID/ }), { target: { value: "private-model" } });
    fireEvent.change(within(dialog).getByRole("textbox", { name: /显示名称/ }), { target: { value: "私有模型" } });
    fireEvent.click(within(dialog).getByRole("button", { name: "添加模型" }));

    await waitFor(() => expect(screen.getAllByText("私有模型").length).toBeGreaterThanOrEqual(1));
  });
  ...
  it("loads an existing gateway token hidden and toggles its visibility", async () => {
    ...
    await waitFor(() => expect(screen.getAllByText("Sub2API").length).toBeGreaterThan(0));
  ...
  it("toggles a configuration target from the full card", async () => {
    ...
    await waitFor(() => expect(checkbox).toHaveProperty("checked", !initialState));
  ...
  it("guards unsaved model edits before changing the active model", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /Claude Sonnet 4\.5 claude-sonnet-4-5/ }));

    let dialog = screen.getByRole("dialog", { name: "丢弃未保存的更改" });
    fireEvent.click(within(dialog).getByRole("button", { name: "取消" }));
    expect(document.querySelector(".model-summary h3")).toHaveTextContent("GPT-5.6");

    fireEvent.click(screen.getByRole("button", { name: /Claude Sonnet 4\.5 claude-sonnet-4-5/ }));
    dialog = screen.getByRole("dialog", { name: "丢弃未保存的更改" });
    fireEvent.click(within(dialog).getByRole("button", { name: "丢弃更改" }));
    await waitFor(() => expect(document.querySelector(".model-summary h3")).toHaveTextContent("Claude Sonnet 4.5"));
  });
  ...
  it("does not change the active model when selecting models for publish", async () => {
    ...
    await waitFor(() => expect(screen.getAllByText("GPT-5.6").length).toBeGreaterThan(0));
    fireEvent.click(screen.getByRole("checkbox", { name: /模型 Claude Sonnet 4\.5/ }));
  ...
  it("hydrates checked and indeterminate selection without publishing partial matches", async () => {
    ...
    vi.spyOn(api, "bootstrap").mockResolvedValueOnce({
      ...data,
      settings: { ...data.settings, selectedTargets: ["workbuddy", "codebuddy"] },
      targetModelStates: [
        {
          target: "workbuddy",
          fingerprint: "work",
    ...
    const checked = await screen.findByRole("checkbox", { name: "选择模型 GPT-5.6" });
    const partial = screen.getByRole("checkbox", { name: /模型 Claude Sonnet 4\.5 已存在于 WorkBuddy/ });
    ...
    expect(preparePublish).toHaveBeenCalledWith(expect.objectContaining({ modelIds: ["gpt-5.6"] }));
  });

  it("promotes an indeterminate model to checked for all selected targets", async () => {
    render(<App />);

    const partial = await screen.findByRole("checkbox", { name: /模型 Claude Sonnet 4\.5 已存在于 WorkBuddy/ });
    expect(partial).toHaveAttribute("aria-checked", "mixed");
    fireEvent.click(partial);

    expect(screen.getByRole("checkbox", { name: "选择模型 Claude Sonnet 4.5" })).toHaveAttribute("data-state", "checked");
  });

  it("shows a filter-specific empty state and clears all filters", async () => {
    render(<App />);

    const search = await screen.findByRole("searchbox", { name: "搜索名称、模型 ID 或提供商" });
    fireEvent.change(search, { target: { value: "no-such-model" } });

    expect(await screen.findByRole("heading", { name: "没有匹配的模型" })).toBeInTheDocument();
  ...
  it("announces a localized error once and includes recovery guidance", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "手动添加模型" }));
    const dialog = screen.getByRole("dialog", { name: "手动添加模型" });
    fireEvent.change(within(dialog).getByRole("textbox", { name: /Model ID/ }), { target: { value: "gpt-5.6" } });
    fireEvent.click(within(dialog).getByRole("button", { name: "添加模型" }));
    ...
    expect(screen.getByRole("status")).not.toHaveTextContent("无法提交当前内容");
  });
  ...
  it("uses app dialogs for destructive confirmations", async () => {
    ...
    expect(screen.getByRole("dialog", { name: "恢复配置备份" })).toBeInTheDocument();
  ...
  it("excludes unavailable targets from publish requests", async () => {
    ...
    vi.spyOn(api, "bootstrap").mockResolvedValueOnce({
      ...data,
      targets: data.targets.map((target) => target.kind === "codebuddy" ? { ...target, writable: false } : target),
      settings: { ...data.settings, selectedTargets: ["workbuddy", "codebuddy"] },
    });
    const preparePublish = vi.spyOn(api, "preparePublish");
    render(<App />);

    const modelCheckbox = await screen.findByRole("checkbox", { name: "选择模型 GPT-5.6" });
    fireEvent.click(modelCheckbox);
    fireEvent.click(screen.getByRole("button", { name: /预览并发布/ }));

    await waitFor(() => expect(preparePublish).toHaveBeenCalled());
    expect(preparePublish).toHaveBeenCalledWith(expect.objectContaining({ targets: ["workbuddy"] }));
  });
  ...
  it("keeps newly saved settings when target availability is refreshed", async () => {
    ...
    const data = {
      ...
      settings: {
        ...current.settings,
        selectedTargets: ["workbuddy", "codebuddy"] as Array<"workbuddy" | "codebuddy">,
      },
    };
    vi.spyOn(api, "bootstrap").mockResolvedValueOnce(data);
    vi.spyOn(api, "getTargetStatuses").mockResolvedValue(
      data.targets.map((target) => target.kind === "codebuddy" ? { ...target, writable: false } : target),
    );
    vi.spyOn(api, "getTargetModelStates").mockResolvedValue(data.targetModelStates);
    const saveSettings = vi.spyOn(api, "saveSettings").mockImplementation(async (settings) => settings);
    render(<App />);
    ...
    expect(saveSettings.mock.calls[saveSettings.mock.calls.length - 1]?.[0]).toMatchObject({
      theme: "dark",
  ...
  it("disables conflicting API and model changes while a gateway is refreshing", async () => {
    let finishRefresh: ((models: Awaited<ReturnType<typeof api.discoverModels>>) => void) | undefined;
    vi.spyOn(api, "discoverModels").mockImplementation(() => new Promise((resolve) => {
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: "手动添加模型" })).toBeDisabled());
