...
显示「暂时无法查询 OpenRouter」时，检查网络后选择「重新查询 OpenRouter」。这不表示模型不存在；主动重试不受自动请求的退避时间限制。

## Target 不可发布

在「设置」中检查 WorkBuddy 和 CodeBuddy 的配置路径。确认当前用户能够访问父目录和 `models.json`。损坏的 JSON、dangling symlink、无写权限或超过大小限制的文件会停止发布。

旧版本保存的相对路径需要重新保存；建议填写绝对路径。发布还会校验合并后的文件，现有模型与新模型的总数不能超过 10,000，最终文件不能超过 8 MiB。

## 上次发布被中断

启动后应用会尝试恢复未完成的写入。显示「已恢复写入前的文件」时，重新预览并发布。显示外部修改提示时，先检查目标文件；应用保留外部更改，不自动覆盖。恢复失败时，检查目标路径和权限后使用「重试恢复写入」；仍失败时再重新打开应用。

Target 中出现未匹配模型时，可以在「发布目标」区域选择「清理未匹配模型」。EveryBuddy 会先创建备份，再移除当前无法匹配本地 API 来源的模型；无法解析的条目会保留并继续显示为异常。

恢复旧备份前，确认当前 Target 的配置路径与备份列表中的路径相同。路径已经变更或被另一个 Target 使用时，应用拒绝恢复，避免改写错误文件。
...
更新安装完成但重启失败时，选择「重启应用」。该操作只重启，不会重新下载安装包。更新前保存模型更改；存在未保存编辑时，应用会先要求确认。
