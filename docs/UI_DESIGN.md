...
首次加载只预选已检测、可写且 schema 有效的目标；后续记忆用户选择。右栏底部显示全局「Model ID 数 → 目标数」及来源数量，Primary Action 固定在 Command Bar。按钮统计跨 API 来源、去重后的 Model ID 数；发现的全部模型不会自动加入发布。左侧 API 切换仅改变浏览和编辑范围，每个来源显示自己的已选数量。列表的「全选」和「清除」只作用于当前来源。
...
同一 Model ID 在多个来源被勾选时，先显示来源选择 Dialog：按 Model ID 分组，列出候选来源名称和 API URL，使用 Radio Group 要求逐项选择。未完成全部选择时，「继续预览」不可用。取消不改变模型勾选状态。

Preview Dialog 先按来源列出最终发布的 Model ID，再分目标显示路径、新增、更新、移除和不变数量。来源选择为空时明确提示其已管理项将按预览移除或替换。目标文件已有同 ID 配置时，同时展示现有名称与本次来源，主按钮保持不可用，直到用户勾选替换确认。取消某来源的最后一个模型后，仍允许预览该来源的移除操作。
